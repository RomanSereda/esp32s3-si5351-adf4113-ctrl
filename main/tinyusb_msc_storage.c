...
#ifdef CONFIG_ESP32_STORAGE_MEDIA_SDMMCCARD
...
static int console_write(int argc, char **argv)
{
    ...
    if (!fd) {
        ...
        fprintf(fd, "In this ESP32, ESP chip will be recognised by host (PC) as Mass Storage Device.\n");
        fprintf(fd, "Upon connection to USB host (PC), the ESP32 application will initialize the storage module and then the storage will be seen as removable device on PC.\n");
        fclose(fd);
    }
    return 0;
}
...
#ifdef CONFIG_ESP32_STORAGE_MEDIA_SPIFLASH
static esp_err_t storage_init_spiflash(wl_handle_t *wl_handle, const esp_partition_t *data_partition)
{
...
#else  // CONFIG_ESP32_STORAGE_MEDIA_SPIFLASH
static esp_err_t storage_init_sdmmc(sdmmc_card_t **card)
{
    ...
    // ESP32: for fixed frequency of 10MHz, use host.max_freq_khz = 10000;
...
#ifdef CONFIG_ESP32_SDMMC_BUS_WIDTH_4
    slot_config.width = 4;
#else
    slot_config.width = 1;
#endif  // CONFIG_ESP32_SDMMC_BUS_WIDTH_4

    // On chips where the GPIOs used for SD card can be configured, set the user defined values
#ifdef CONFIG_SOC_SDMMC_USE_GPIO_MATRIX
    slot_config.clk = CONFIG_ESP32_PIN_CLK;
    slot_config.cmd = CONFIG_ESP32_PIN_CMD;
    slot_config.d0 = CONFIG_ESP32_PIN_D0;
#ifdef CONFIG_ESP32_SDMMC_BUS_WIDTH_4
    slot_config.d1 = CONFIG_ESP32_PIN_D1;
    slot_config.d2 = CONFIG_ESP32_PIN_D2;
    slot_config.d3 = CONFIG_ESP32_PIN_D3;
#endif  // CONFIG_ESP32_SDMMC_BUS_WIDTH_4
    ...
    // connected on the bus. This is for debug / ESP32 purpose only.
...
const esp_partition_t* tinyusb_msc_storage(void)
{
    ...
    ESP_ERROR_CHECK(storage_init_spiflash(&wl_handle, data_partition));
